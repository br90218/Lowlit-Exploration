Shader "Horizon[ON]/Horizon[ON] Tessellation(DX11)" {
    Properties {
[BlockInfo(1.0,1.0,1.0,1.0)] dummy_begin ("Features", Float) = 1
	    [KeywordEnum(One, Two, Three, Four)] _LayerCount ("   Layer count", Float) = 3
	    [HideInInspector][KeywordEnum(Ambient, Cubemap, SkyshopSH, Unity5SH)] _IBLDiff ("   Diffuse Ambient Lighting Mode", Float) = 3
...
[BlockInfo(0.2,0.7,1.0,1.0, 1, _Water)] dummy_water ("Water features", Float) = 1
        ...
        [HideByProp(dummy_water, _Water)] _WaterNormalmap ("   Normalmap", 2D) = "bump" {}
...
[BlockInfo(1.0,1.0,1.0,1.0, 1, indent)] dummy_displacement ("Displacement", Float) = 1
		[HideByProp(dummy_displacement)] _Parallax ("   Height [m]", Float ) = 100
		//_EdgeLength ("Requested edge length [pixels]", Range(1,50) ) = 5
		[HideByProp(dummy_displacement)] _EdgeLength ("   Subdivisions", Range(1,40) ) = 5
		[HideByProp(dummy_displacement,0)] _ParallaxMap ("   Heightmap (A/R/G/B)", 2D) = "black" {}
		[PropBlock(dummy_displacement)] _ReduceByVertexAlpha ("       Flatten by Vertex Color (A)", Float) = 0
		[PropBlock(dummy_displacement)] _ReduceByUVBorder ("       Flatten by UVs (near 0 and 1)", Float) = 0		
		[HideByProp(dummy_displacement, _ReduceByUVBorder)] _ReduceByUVBorderLength ("       Flatten Strength", Range(0,0.95)) = 0.5    
    }
    ...
    SubShader {
        ...
        Pass {
			...
			#define USE_UVS_FOR_DISPLACEMENT_BLEND
...
//            #define SHADOWS_OFF
            ...
            //#define _CLIFFS
            //#define _HORIZONDISK
            //#pragma multi_compile _CLIFFDETAIL_NONE _CLIFFDETAIL_COLOR _CLIFFDETAIL_NORMALS
...
//            #pragma shader_feature _SNOW_OFF _SNOW_ON
    ...
    FallBack "Horizon[ON]/Horizon[ON] Displacement"
}