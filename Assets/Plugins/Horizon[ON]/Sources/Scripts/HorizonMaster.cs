...
namespace Horizon {
	...
	public class HorizonMaster : MonoBehaviour {
		...
		[HideInInspector] public bool showMaterials = false;
		...
		[HideInInspector] public bool showIBLSettings = false;
		...
		[HideInInspector] public enum DiffLightMode {Ambient, CubeM, SkyshopSH, Unity5SH};
		[HideInInspector] public DiffLightMode hFeatDiffLightMode = DiffLightMode.Ambient;
        ...
        [HideInInspector] public float hSetScaleInner = 0.9f;
		[HideInInspector] public float hSetScaleOuter = 0;
		[HideInInspector] public float hSetScaleHeight = 0.2f;
		// Settings Main
		...
		[HideInInspector] public float hSetAmbvsIBL = 1;
		[HideInInspector] public float hSetNMIntensLayers = 1;
		...
		[HideInInspector] public float hSetDetailDiffIntens = 0.1f;
		[HideInInspector] public float hSetDetailNMIntens = 1;
		// Settings Water
		...
		[HideInInspector] public float hSetWaterBlend = 0.98f;
		[HideInInspector] public float hSetWaterWavesIntens = 0.15f;
		[HideInInspector] public float HSetWaterWavesSpeed = 1;
		// Settings Fog
		[HideInInspector] public float hSetFogIntens = 1;
		[HideInInspector] public Color hColFogColorAmbBlend = new Color32(223,230,235,0);
		[HideInInspector] public float hSetFogSpecCubeAdd = 0.3f;
		...
		[HideInInspector] public float hSetFogEmissPunchThru = 0.1f;
		// Settings Snow
		[HideInInspector] public float hsetSnowAmount = 1;
		...
		[HideInInspector] public float hSetSnowSlopeDamp = 0.75f;
		[HideInInspector] public float hSetSnowReduceByColor = 0.75f;
		// Settings DiffCube, Skyshop SH, Unity5 SH
		[HideInInspector] public Cubemap hCubeIBLDiff;
		[HideInInspector] public float hSetIBLExposure = 0.6f;
		// Settings SpecCube
		[HideInInspector] public Cubemap hCubeIBLSpec;
		[HideInInspector] public float hSetSpecRoughMIPLevel = 5;
		[HideInInspector] public float hSetSpecRoughMulti = 0.2f;
		[HideInInspector] public float hSetSpecGlossMIPLevel = 0.2f;
		[HideInInspector] public float hSetSpecGlossMulti = 1;
		...
		[HideInInspector] public float hSetDispRedFadeAmount = 1;
		[HideInInspector] public Texture hTexDispHeightmap;
		[HideInInspector] public float hSetTessSubD = 0;
		// Settings Cliffs
		[HideInInspector] public Texture hTexCliffDiff;
		[HideInInspector] public Vector4 hSetCliffScaleOffset = new Vector4(50,50,0,0);
		[HideInInspector] public bool hSetCliffAIsGlossMask = false;
		[HideInInspector] public bool hSetCliffAIsEmissMask = false;
		[HideInInspector] public Color hColCliffSpecGloss = new Color(0,0,0,0);
		[HideInInspector] public Texture hTexCliffNM;
		[HideInInspector] public enum HSetCliffDetailMode {None, Color, Normal};
		[HideInInspector] public HSetCliffDetailMode hSetCliffDetailMode = HSetCliffDetailMode.None;
		[HideInInspector] public Texture hTexCliffDetDiff;
		[HideInInspector] public Vector4 hSetCliffDetScaleOffset = new Vector4(220,220,0,0);
		[HideInInspector] public Texture hTexCliffDetNM;
		[HideInInspector] public float hSetCliffDetDiffIntens = 0.2f;
		[HideInInspector] public float hSetCliffDetNMIntens = 1;
		...
		public void ShowWireFrame (bool active){
			foreach (MeshRenderer mr in GetComponentsInChildren<MeshRenderer>()) {
				UnityEditor.EditorUtility.SetSelectedWireframeHidden (mr, active);
			}
		}
		...
		public void CheckMaterials(){
			...
			foreach (MeshRenderer mr in GetComponentsInChildren<MeshRenderer>()) {
				...
				if (mr.sharedMaterial.HasProperty ("_CliffDetailmap")) {
				...
				if (m != null){
					...
					if (setFeatures){
						...
						if (m.HasProperty ("_IBLDiff")){
							switch (hFeatDiffLightMode){
							case DiffLightMode.Ambient : m.SetFloat ("_IBLDiff",0); m.EnableKeyword("_IBLDIFF_AMBIENT"); m.DisableKeyword("_IBLDIFF_CUBEMAP"); m.DisableKeyword("_IBLDIFF_SKYSHOPSH"); m.DisableKeyword("_IBLDIFF_UNITYSH"); break;
							case DiffLightMode.CubeM : m.SetFloat ("_IBLDiff",1); m.DisableKeyword("_IBLDIFF_AMBIENT"); m.EnableKeyword("_IBLDIFF_CUBEMAP"); m.DisableKeyword("_IBLDIFF_SKYSHOPSH"); m.DisableKeyword("_IBLDIFF_UNITYSH"); break;
							case DiffLightMode.SkyshopSH : m.SetFloat ("_IBLDiff",2); m.DisableKeyword("_IBLDIFF_AMBIENT"); m.DisableKeyword("_IBLDIFF_CUBEMAP"); m.EnableKeyword("_IBLDIFF_SKYSHOPSH"); m.DisableKeyword("_IBLDIFF_UNITYSH"); break;
							case DiffLightMode.Unity5SH : m.SetFloat ("_IBLDiff",3); m.DisableKeyword("_IBLDIFF_AMBIENT"); m.DisableKeyword("_IBLDIFF_CUBEMAP"); m.DisableKeyword("_IBLDIFF_SKYSHOPSH"); m.EnableKeyword("_IBLDIFF_UNITYSH"); break;
							}
						}
					...
					if (m.HasProperty ("_BaseGloss")) { if (hSetLayerProps[0].hSetAIsGlossMask) m.SetFloat ("_BaseGloss", 1); else m.SetFloat ("_BaseGloss", 0); }
					...
					if (m.HasProperty ("_SpecGloss")) { m.SetColor ("_SpecGloss", hSetLayerProps[0].hColLayerSpecGloss); }
					...
					if (m.HasProperty ("_BlendGloss1")) { if (hSetLayerProps[1].hSetAIsGlossMask) m.SetFloat ("_BlendGloss1", 1); else m.SetFloat ("_BlendGloss1", 0); }
					...
					if (m.HasProperty ("_SpecGlossBlend1")) { m.SetColor ("_SpecGlossBlend1", hSetLayerProps[1].hColLayerSpecGloss); }
					...
					if (m.HasProperty ("_BlendGloss2")) { if (hSetLayerProps[2].hSetAIsGlossMask) m.SetFloat ("_BlendGloss2", 1); else m.SetFloat ("_BlendGloss2", 0); }
					...
					if (m.HasProperty ("_SpecGlossBlend2")) { m.SetColor ("_SpecGlossBlend2", hSetLayerProps[2].hColLayerSpecGloss); }
					...
					if (m.HasProperty ("_BlendGloss3")) { if (hSetLayerProps[3].hSetAIsGlossMask) m.SetFloat ("_BlendGloss3", 1); else m.SetFloat ("_BlendGloss3", 0); }
					...
					if (m.HasProperty ("_SpecGlossBlend3")) { m.SetColor ("_SpecGlossBlend3", hSetLayerProps[3].hColLayerSpecGloss); }
					...
					// IBL Settings =============================================================================================================================
					if (m.HasProperty ("_DiffuseIBLCubemap")) { m.SetTexture ("_DiffuseIBLCubemap", hCubeIBLDiff); }
					if (m.HasProperty ("_DiffIBLMulti")) { m.SetFloat ("_DiffIBLMulti", hSetIBLExposure); }
					if (m.HasProperty ("unity_SpecCube0")) { m.SetTexture ("unity_SpecCube0", hCubeIBLSpec); }
					if (m.HasProperty ("_IBLSpecRoughMIP")) { m.SetFloat ("_IBLSpecRoughMIP", hSetSpecRoughMIPLevel); }
					if (m.HasProperty ("_IBLMIPDim_Rough")) { m.SetFloat ("_IBLMIPDim_Rough", hSetSpecRoughMulti); }
					if (m.HasProperty ("_IBLSpecGlossMIP")) { m.SetFloat ("_IBLSpecGlossMIP", hSetSpecGlossMIPLevel); }
					if (m.HasProperty ("_IBLMIPDim_Gloss")) { m.SetFloat ("_IBLMIPDim_Gloss", hSetSpecGlossMulti); }
					...
					if (m.HasProperty ("_CliffGloss")) {if (hSetCliffAIsGlossMask) m.SetFloat ("_CliffGloss", 1); else m.SetFloat ("_CliffGloss", 0); }
					if (m.HasProperty ("_CliffEmission")) { if (hSetCliffAIsEmissMask) m.SetFloat ("_CliffEmission", 1); else m.SetFloat ("_CliffEmission", 0); }
					if (m.HasProperty ("_SpecGlossCliff")) { m.SetColor ("_SpecGlossCliff", hColCliffSpecGloss); }
					if (m.HasProperty ("_CliffNormalmap")) { m.SetTexture ("_CliffNormalmap", hTexCliffNM); }
					if (m.HasProperty ("_CliffDetail")) { 
						switch (hSetCliffDetailMode){
						case HSetCliffDetailMode.None : m.SetFloat ("_CliffDetail", 0); m.EnableKeyword("_CLIFFDETAIL_NONE"); m.DisableKeyword("_CLIFFDETAIL_COLOR"); m.DisableKeyword("_CLIFFDETAIL_NORMALS"); break;
						case HSetCliffDetailMode.Color : m.SetFloat ("_CliffDetail", 1); m.DisableKeyword("_CLIFFDETAIL_NONE"); m.EnableKeyword("_CLIFFDETAIL_COLOR"); m.DisableKeyword("_CLIFFDETAIL_NORMALS"); break;
						case HSetCliffDetailMode.Normal : m.SetFloat ("_CliffDetail", 2); m.DisableKeyword("_CLIFFDETAIL_NONE"); m.DisableKeyword("_CLIFFDETAIL_COLOR"); m.EnableKeyword("_CLIFFDETAIL_NORMALS"); break;
						}
					}
					if (m.HasProperty ("_CliffDetailmap")) { m.SetTexture ("_CliffDetailmap", hTexCliffDetDiff); }
					if (m.HasProperty ("_CliffDetailmap")) { m.SetTextureScale ("_CliffDetailmap", new Vector2(hSetCliffDetScaleOffset.x, hSetCliffDetScaleOffset.y)); }
					if (m.HasProperty ("_CliffDetailmap")) { m.SetTextureOffset ("_CliffDetailmap", new Vector2(hSetCliffDetScaleOffset.z, hSetCliffDetScaleOffset.w)); }
					if (m.HasProperty ("_CliffDetailColorIntensity")) { m.SetFloat ("_CliffDetailColorIntensity", hSetCliffDetDiffIntens); }
					if (m.HasProperty ("_CliffDetailNormalmap")) { m.SetTexture ("_CliffDetailNormalmap", hTexCliffDetNM); }
					if (m.HasProperty ("_CliffDetailNormalIntensity")) { m.SetFloat ("_CliffDetailNormalIntensity", hSetCliffDetNMIntens); }
				}
			}
		}
	...
	public class HorizON_LayerProps {
		...
		public bool hSetAIsGlossMask = false;
		public bool hSetAIsEmissMask = false;
		public Color hColLayerTint = new Color32(128,128,128,128);
		public Color hColLayerSpecGloss = new Color(0,0,0,0);
		public float hSetLayerDetIntens = 1;
    #endif
    }
}