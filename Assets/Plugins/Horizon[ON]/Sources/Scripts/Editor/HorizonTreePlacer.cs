...
namespace Horizon
{
    public class HorizonTreePlacer : EditorWindow {
        ...
        void DrawObjectModeUI()
        {
            if (hdps.parent == null)
            {
                if (prefabs.Count == 1) EditorGUILayout.HelpBox("Add Prefabs in the Prefabslot above or use the \"Load Prefab List\" Button", MessageType.Info);
            ...
            else
            {
                ...
                //--->
                    EditorGUILayout.BeginHorizontal();
                    Rect r = new Rect(EditorGUILayout.GetControlRect(GUILayout.MaxWidth((Screen.width / 3) - 6), GUILayout.Height(17)));
                    EditorGUI.HelpBox(r, "Objects: " + hdps.currentObjectCount.ToString(), MessageType.None);
                    r = new Rect(EditorGUILayout.GetControlRect(GUILayout.MaxWidth((Screen.width / 3) - 6), GUILayout.Height(17)));
        ...
        //--->
            ...
            //---> Scroll View ----------------------------------------------------------------
                ...
                //--->
                    ...
                    prefabs[i] = EditorGUILayout.ObjectField(prefabs[i], typeof(GameObject), false, GUILayout.Width(Screen.width - space)) as GameObject;
                    if (i != prefabs.Count - 1)
                    {
        ...
        void CloseGroup()
        {
            if (hdps.parent != null)
            {
                ...
                mr.reflectionProbeUsage = UnityEngine.Rendering.ReflectionProbeUsage.Simple;
        ...
        void StoreValuesInParent(GameObject group)
        {
            ...
            for (int i = 0; i < hdps.parent.transform.childCount; i++)
            {
                ...
                groupParent.gameObjects.Add(PrefabUtility.GetPrefabParent(t.gameObject) as GameObject);
