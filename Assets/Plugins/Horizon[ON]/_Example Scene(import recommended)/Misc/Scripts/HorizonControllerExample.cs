...
public class HorizonControllerExample : MonoBehaviour
{
    ...
    float displacementAmount, globalNormalMapIntensity, detailNormalMapIntensity, fogAmount, snowAmount, waterWaves; // The current displacement/bumpmap intensity, fog and snow amount values.

    // The Original Values
    Vector4 mOrig, l1Orig, l2Orig, l3Orig, l4Orig;
    float dispOrig, gnmOrig, dnmOrig, fogOrig, snowOrig, wavesOrig;
    ...
    public void Reset()
    {
        if (horizonDisplaced != null) horizonDisplaced.SetActive(true);
